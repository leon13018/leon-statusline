...
- **仍套用的理由**：旗標生效有硬證據（B 組無 `typingsInstaller`）；9 個受害專案**根目錄**皆無
  `package.json` / `node_modules`，停用 ATA **功能損失趨近於零**（非嚴格為零 —— 其中 7 個在深層
  子目錄仍有 `package.json`，但都不在被編輯檔案的祖先鏈上，詳見 spec §3.4）；
  實測仍省 22.7%；單行設定、已備份、可逆。
